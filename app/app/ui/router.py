...
def download_windows_agent_script() -> FileResponse:
    candidates = [
        Path(__file__).resolve().parents[2] / "dist" / "windows" / "install_agent.ps1",
        Path(__file__).resolve().parents[3] / "agent" / "windows" / "install_agent.ps1",
    ]
    ...
    raise HTTPException(
        status_code=404,
        detail="Windows installer script not found. Expected dist/windows/install_agent.ps1.",
    )
